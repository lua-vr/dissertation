...
\begin{remark}[A different proof]
  ...
  \begin{align*}
    ...
    &= \set[par = auto]{x \in X : \liminf_{n \to \infty} \sup_t\abs*{\frac{\PSum[\floor{nt} + 1]{x} - p(x)}{\sqrt{n}} - p_{\delta}(t)} <\delta }\\
    &= \set[par = auto]{x \in X : \liminf_{n \to \infty} \sup_t\abs*{\frac{\PSum[\floor{nt'}]{x}}{\sqrt{n}} - p_{\delta}(t)} <\delta }  \text{ where }t' = t + 1/n\\
    &= \set[par = auto]{x \in X : \liminf_{n \to \infty} \sup_t\abs*{\frac{\PSum[\floor{nt}]{x}}{\sqrt{n}} - p_{\delta}(t)} <\delta } = S_{\delta}.
  \end{align*}
...
\section{Recent developments}
  ...
  \todo{Conclude explaining the recent work of Kiriri, Soma, Vargas et al.}
