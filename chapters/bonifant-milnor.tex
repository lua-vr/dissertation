...
\begin{proof}
  We will prove the case \(i = 0\), as the situation is symmetric. Let \((x,y_1) \in \Basin{\mu_0}\) and \(y_2 \leq y_1\). If \(\varphi \in \Cs{X \times I}\), and \(\varepsilon > 0\), by the uniform continuity of \(\varphi\), there exists \(\delta > 0\) such that \(y < \delta\) implies \(\lvert \varphi(x,0) - \varphi(x,y) \rvert < \varepsilon\) for all \(x \in X\). Since \(U = X \times \interval[open right]{0}{\delta}\) is a continuity set for \(\mu_0\) and \(e_n(x,y_2) \to \mu_0\), by \nameref{thm:portmanteau}
...
\todo{make the link to \cref{sec:lyapunov-exponents}}
...
\begin{remark}
  With Pesin theory, it is possible to show that intermingled basins are present in the more general setting of \(g\) being a hyperbolic automorphism. See the examples 3.2.1 and 3.2.2 of~\cite{uresNonrobustnessIntermingledBasins2018}. The argument goes as follows: pick a point \(p = (x,y)\) with \(y \in (0,1)\), and consider a disk \(D_p\) transversal to the center direction (the \(y\) direction).
  \todo{todo}
\end{remark}
...
\begin{proof}
  ...
  \begin{equation*}
    \xi_0(x) = \sup \{ y \in I : (x,y) \in \mathcal{B}_0 \}, \qquad \xi_1(x) = \inf \{ y \in I : (x,y) \in \mathcal{B}_1 \}.
  \end{equation*}
...
\begin{question}
  We know the asymptotic measure \(\nu\) of \cref{thm:pos-schwarz-asymptotic} satisfies \((\pi_X)_*\nu = \mu\), and we can disintegrate \(\nu = \int \nu_x \;d\mu(x)\) for the partition \(\set{x}\times I\). What are general conditions for \(\nu_x \ll \Leb\) (that is, when is \(\nu\) absolutely continuous on the fibers)?
\end{question}
