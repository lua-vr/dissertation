...
\begin{hypothesis}[Elevators]\label{hip:elevators}
  There exist two fixed points \(x^-\) and \(x^+\) in \(X\) whose unstable manifolds are dense, and such that for every  \(y \in (0,1)\), \(f_{x^-}(y) <  y\) and \(f_{x^+}(y) > y\).
\end{hypothesis}

\begin{figure}[h]
  \centering
  \includegraphics[width=.6\textwidth]{../figures/elevators.png}
  \caption{Elevators (TODO: make my own figure)}
  \label{fig:elevators}
\end{figure}

\begin{theorem}
  Suppose \cref{hip:elevators} holds. Then, any closed strictly invariant set  \(K \subseteq X \times I\) with at least one point \((x,y) \in K\) such that \(y \in (0,1)\) contains both boundaries \(X \times \set{0,1}\).
\end{theorem}
...
\begin{theorem}\label{thm:neg-schwarz-sep-graph}
  If there exists a function \(r \colon I \times I \to \mathbb{R}\)
  such that \[r(y_1,y_2) < r(f_x(y_1),f_x(y_2))\] for every \(y_1 < y_2\) and \(\mu\)-almost every \(x \in X\),
...
\begin{remark}
  The proofs of \cref{thm:neg-schwarz-sep-graph,thm:pos-schwarz-asymptotic} would carry out mostly unchanged if instead of assuming negative Schwarzian for the fiber maps, we assumed that one basin has positive measure and that for every \(x\) there is a metric \(d_x\) in \(I\), equivalent and uniformly stronger than the Euclidian metric, with the property that \(d_x(y_1,y_2) < d_{g(x)}(f_x(y_1),f_x(y_2))\). In the negative Schwarzian case, this metric is the hyperbolic metric.
\end{remark}
