...
\begin{proposition}\label{prop:schwarzian}
  ...
  \begin{enumerate}
  \item\label{prop:schwarzian:item:1} If \(\mathcal{S}f \succ 0\) and \(\mathcal{S}g \succ 0\), then \(\mathcal{S}(f \circ g) \succ 0\), and similarly for \(\prec 0\). Moreover, if \(\mathcal{S}f \equiv 0\), then \(\SchD{f \circ g} = \SchD{g}\).
...
\begin{lemma}\label{lem:sch-fixed-pts}
  If \(f \colon I \to I'\) is \(C^3\) and \(\SchD f \prec 0\), then \(f\) has at point three fixed points. When it has three fixed points, the middle one is strictly repelling and the other two are strictly attracting.
\end{lemma}
