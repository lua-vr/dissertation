...
\begin{proposition}\label{prop:schwarzian}
  ...
  \begin{enumerate}
  \item If \(\mathcal{S}f \succ 0\) and \(\mathcal{S}g \succ 0\), then \(\mathcal{S}(f \circ g) \succ 0\), and similarly for \(\prec 0\). Moreover, if \(\mathcal{S}f \equiv 0\), then 
...
\begin{proof}\hfill
  ...
  \item We have
    ...
    \begin{equation}\label{sch-comp}
      ...
      \end{aligned}
    \end{equation}
    Since \(g\) is a diffeomorphism, \(g'(x) \neq 0\) for all \(x \in I\), and therefore \((g')^2 > 0\). In particular, \(g\) is a homeomorphism, so that if \(\SchD f(x) > 0\) for all \(x\) the open dense set \(U\), and \(\SchD g(x) > 0\) in the open dense set \(V\), then \(\SchD{f \circ g} > 0\) in the open dense set \(g^{-1}(U) \cap V\). 
...
\begin{proof}
  Suppose \(\SchD f \prec 0\). Given \(y_1 < y_2 < y_3 < y_{4}\), let \(\varphi\) be the linear fractional transformation that sends \(f(y_1)\) to \(y_1\), \(f(y_2)\) to \(y_2\) and \(f(y_4)\) to \(y_4\). Since fractional linear transformations preserve cross-ratios, \(\varphi()\)
\end{proof}
