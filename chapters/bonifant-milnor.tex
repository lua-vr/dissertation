...
In this chapter, we follow the exposition by \textcite{bonifantSchwarzianDerivativesCylinder2008} on a family of endomorphisms of a cylinder with interesting properties.
...
It is not possible to define such operations everywhere without introducing inconsistent rules, so the operations are partial --- we cannot multiply \(\infty \cdot 0\) or add \(\infty + \infty\).
...
This group is a quotient of \(\GL_2(\mathbb{K})\) by a subgroup of dimension \(1\), so it has dimension \(3\). Furthermore, it is \(3\)-transitive: given two sets of three distinct points in \(P^1(\mathbb{K})\), there exists an element \(g \in \PGL_2(\mathbb{K})\) that takes the first set to the second. But not only that, \(P^1(\mathbb{K})\) has three special points: \(0\), \(1\), and \(\infty\). Thus, given \(x_1,x_2, x_3 \in P^1(\mathbb{K})\) distinct, we can ask which \(g \in \PGL_2(\mathbb{K})\) is such that \(g(x_1) = \infty\), \(g(x_2) = 0\), and \(g(x_3) = 1\). With some calculations, we arrive at
\begin{equation*}
...
The Schwarzian derivative has some important properties that we highlight in the Proposition below, as described in~\cite[Proposition B.1]{bonifantSchwarzianDerivativesCylinder2008}.
...
\begin{proof}\hfill
  ...
  \item We have
    ...
    \end{equation*}
    By the Faà di Bruno's (beautiful) formula~\cite[479]{tortoliniAnnaliDiScienze1855} in its combinatorial form,
    \begin{equation*}
...
\begin{lemma}\label{lem:basin-segment}
  If \((x, y_1) \in \Basin{\mu_0}\), then \((x,y_2) \in \Basin{\mu_0}\) for all \(y_2 \leq y_1\). Similarly, if \((x, y_1) \in \Basin{\mu_1}\), then \((x,y_2) \in \Basin{\mu_1}\) for all \(y_2 \geq y_1\).
\end{lemma}

\begin{proof}
  We will prove the case \(i = 0\), as the situation is symmetric. Let \((x,y_1) \in \Basin{\mu_0}\) and \(y_2 \leq y_1\). If \(\varphi \in \Cs{X \times I}\), and \(\varepsilon > 0\), by the uniform continuity of \(\varphi\), there exists \(\delta > 0\) such that \(y < \delta\) implies \(\lvert \varphi(x,0) - \varphi(x,y) \rvert < \varepsilon\) for all \(x \in X\). Since \(U = X \times \interval[open right]{0}{\delta}\) is a continuity set for \(\mu_0\) and \(e_n(x,y_2) \to \mu_0\), by \cref{thm:portmanteau}
    \newcommand{\timeinU}{\frac{1}{n}\# \set{ 0 \leq i < n \where f_x^n(y_2) < \delta }}
    \[\lim_{n \to \infty} e_n(x,y_2)(U) = \mu_0(U) = 1.\]
    ...
    \end{align*}
    Since \(\tau_n \to 1\), there exists \(N\) such that \(n \geq N\) implies that the right side is less than \(2 \varepsilon\), hence both integrals on the left converge to \(\mu_0(\varphi)\).
\end{proof}
...
The statement below assumes less regularity than the corresponding one in~\cite[lemma A.1]{bonifantSchwarzianDerivativesCylinder2008}, that asks for \(F\) to be \(C^2\). The second part of the proof (when the Lyapunov exponent is positive) is also more general, because the basins of the original article are topological instead of measure-theoretic. In our context, \(F(x,y) = (g(x),f_x(y))\) where \(g\) is an ergodic map for \(\mu\) and \(f_x \in C^1[0,1]\) for all \(x\). In particular, if \((x,y) \mapsto f'_x(y)\) is continuous, the following equicontinuity hypothesis is satisfied. 
...
\begin{proof}
  ...
  \end{align*}
  Since \(\lim_{n \to \infty} \sum_{k = 0}^{n - 1}a(g^k(x)) = -\infty\), we have \(f_x^{n + 1}(y) \to 0\). Thus, if \(x \in \mathcal{B}(\mu)\) (which has full measure), then \(e_{n}(x,y) \weakto \mu_0\) whenever \(y \leq \delta e^{-A(x)}\).

  The converse is slightly more tricky, because \((x,y)\) may be in the basin of \(\mu_0\) but \(f_x^n(y)\not\to 0\).\sidetodo{new! :)} Let \(\varepsilon > 0\) be such that
  \begin{equation*}
    \int \log(f_x'(0) - \varepsilon) \;d\mu(x) > 0.
  \end{equation*}
  and let \(M = \max_{(x,y)} \abs{\log (f'_x(y))}\). Define \(a(x) = \log(f_x'(0) - \varepsilon)\), so like before, 
  \begin{equation*}
    \lim_{n \to \infty}\frac{1}{n}\sum_{i = 0}^{n - 1} a(g^i(x)) = 
    \int a \;d\mu > 0,
  \end{equation*}
  for \(x \in E\) in a full measure set, and let \(\delta\) be such that \(0 \le y \le \delta\) implies \(\abs{f_x'(0) - f_x'(y)} < \epsilon\). Let \(\gamma = (\int a \;d\mu)/(8M) < 1\). Take \((x, y) \in \Basin{\mu_0}\) such that \(x \in E\) and \(y \in (0,1)\), which is possible if we assume \(\Basin{\mu_0}\) has positive \(\mu \otimes \Leb\) measure. Now, consider the strip \(S_{\delta} = X \times \interval[open right]{0}{\delta}\). Since \(\mu_0(S_{\delta}) = 1\), \(\mu_0(\partial S_{\delta}) = 0\) and \((x,y) \in \Basin{\mu_0}\), by \cref{thm:portmanteau} there is \(N_1\) for which \(n \ge N_1\) implies \(G_n = \set{0 \le i \le n \where f_x^i(y) < \delta}\) satisfies \(\# G_n / n > 1 - \gamma\). Let \(N_2 \ge N_1\) be such that \(n \ge N_2\) implies \(\frac{1}{n}\sum_{i = 0}^{n - 1} a(g^i(x)) > 
  (\int a \;d\mu)/2\). Then, for \(n \ge N_2\) and any \(z \le y\), we have
  \begin{equation*}
    \frac{1}{n}\log ((f^n_x)'(z))
    = 
    \frac{1}{n}\sum_{i = 0}^{n - 1} \log(f'_{g^i(x)}(f_x^i(z)))
    \ge 
    \frac{1}{n}\sum_{i \in G_n} \log(f'_{g^i(x)}(f_x^i(z))) - \gamma M.
  \end{equation*}
  Now, \(i \in G_n\) implies \(f^i_x(y) < \delta\), which implies \(f^i_x(z) < \delta\) by monotonicity. This, in turn, implies \(\log(f'_{g^i(x)}(f_x^i(z))) \ge \log(f'_{g^i(x)}(0) - \varepsilon) = a(g^i(x))\). Therefore
  \begin{align*}
    \frac{1}{n}\log ((f^n_x)'(z))
    \ge 
    \frac{1}{n}\sum_{i \in G_n} a(g^i(x)) - \gamma M.
    &\ge 
    \frac{1}{n}\sum_{i = 0}^{n - 1} a(g^i(x)) - 2\gamma M\\
    &\ge \rpar[\big]{\int a \;d\mu}/4 > 0.
  \end{align*}
  This means that \((f^n_x)'(z) \to \infty\) uniformly on \(z \le y\) (because neither \(N_2\) nor \(\int a \;d\mu\) depend on \(z \in [0,y]\)). This is impossible, because by the mean value theorem, \(f^n_x(y) = (f^n_x)'(z_n) \cdot y\) for some \(0\le z_n \le y\), while \(f^n_x(y) \le 1\) is bounded.
\end{proof}
...
Assuming this, the main property that will produce the intermingled phenomenon is the following additional hypothesis.
...
Let's define the measures \(\beta_i(A) = m(A \cap \Basin{\mu_i})\). Then the basins are intermingled if, and only if, \(\supp \beta_i = X \times I\) for both \(i = 0,1\). Indeed, both supports being equal to the whole space means precisely that for every open set \(U \subseteq X \times I\), the measures \(\beta_i(U) = m(U \cap \Basin{\mu_i})\) are both positive for the reference measure \(m = \mu \otimes \Leb\).

We will show this is the case for \(\supp \beta_0\), as the proof for \(i = 1\) follows by symmetry. First, we will collect some lemmas.

\begin{lemma}\label{lem:supp-segment}
  If \((x, y_1) \in \supp \beta_0\), then \((x,y_2) \in \supp \beta_0\) for all \(y_2 \leq y_1\).
\end{lemma}
\begin{proof}
  By \cref{lem:basin-segment}, we know this property is true for \(\Basin{\mu_0}\).
  Let \(U = B_{\varepsilon}(x,y_2)\) be a ball centered at \((x, y_2)\) of radius smaller than \(1 - y_1\), and \(V = B_{\varepsilon}(x,y_1)\). Notice that \[V \cap \Basin{\mu_0} \subseteq \set{(u,v + y_1 - y_2) \where (u,v) \in U \cap \Basin{\mu_0}}.\] Since \((x,y_1) \in \supp \beta_0\), and \(m = \mu \otimes \Leb\) is invariant by vertical translations, \[\beta_0(U) = m(U \cap \Basin{\mu_0}) \geq m(V \cap \Basin{\mu_0}) > 0.\]
  Since this is true for all sufficiently small balls \(U\), this shows \((x,y_2) \in \supp \beta_0\).
\end{proof}

\begin{figure}[h]
  \centering
  \includegraphics{../figures/translation}
\end{figure}

\begin{lemma}
  The set \(\supp \beta_0\) is backwards invariant, that is, \(F^{-1}(\supp \beta_0) = \supp \beta_0\).
\end{lemma}
\begin{proof}
  If \(p \in F^{-1}(\supp \beta_0)\) and \(U\) is a neighborhood of \(p\), then since \(F\) is an open map, \(F(U)\) is a neighborhood of \(F(p) \in \supp \beta_0\), so \(\beta_0(F(U)) > 0\). Since \(F\) is Lipschitz, if \(\beta_0(U)\) were \(0\), then \(\beta_0(F(U)) = m(F(U) \cap \Basin{\mu_0}) = m(F(U \cap \Basin{\mu_0})) = 0\), which is not the case, so \(\beta_0(U) > 0\).
\end{proof}

\begin{lemma}
  We have \(m(\supp \beta_0) \ge \beta_0(\supp \beta_0) = \beta_0(X \times I) = m(\Basin{\mu_0}) > 0\).
\end{lemma}

\begin{proof}
  The last value is positive by \cref{hip:basins-support-nonempty}, and the first and last equalities are, respectively, by subadditivity and definition. To show the second equality, let \(C = (\supp \beta_0)^{\complement}\). By definition, for each \(x \in C\), there exists an open neighborhood \(U_x \ni x\) for which \(\beta_0(U_x) = 0\). Since \(X \times I\) is Lindelöf, we can extract a countable subcover \(U_{x_1} \cup U_{x_2} \cup \cdots\) of \(C\). By subadditivity, this shows \(\beta_0(C) = 0\).
\end{proof}

\begin{lemma}\label{lem:basin-pos-full}
  The set \(B_0 = \set{x \in X \where \exists y \in (0,1), \; (x,y) \in \supp \beta_0}\) is nonempty.
\end{lemma}

\begin{proof}
  It suffices to show it has positive measure. For that, let \(0 < \lambda < m(\supp \beta_0)\), and notice that \[m(\supp \beta_0 \setminus X \times [0,\lambda]) \ge m(\supp \beta_0) - \lambda > 0.\]
  Set \(E_{\lambda} = \supp \beta_0 \setminus X \times [0,\lambda]\). Then \(\mu(\pi_X(E_{\lambda})) \ge m(E_{\lambda}) > 0\), and clearly \(\pi_X(E_{\lambda}) \subseteq B_0\).

\end{proof}


\begin{corollary}\label{cor:basins-eq}
  We have \(\supp \beta_0 = X \times I\).
\end{corollary}

\begin{proof}
  Let \(x_0 \in B_0\) and \(y_0 \in (0,1)\) such that \((x_0,y_0) \in \supp \beta_0\).
  Inductively, we can construct a backward orbit \(\cdots \mapsto x_{-2} \mapsto x_{-1} \mapsto x_0\) of \(g\) with the property that \(x_{-n} \to \Orbit{x^-}\). Indeed, since \(x^-\) is an expanding periodic point, there is a neighborhood \(U\) and a partial inverse \(u \colon U \to g^{-1}(U)\) with \(g \circ u = \Id|_U\) for which \(u^n(x) \to \Orbit{x^-}\) for all \(x \in U\). And by exactness, there is some \(N\) for which \(x_0 \in g^N(U)\).


  Since \(x_{-n} \to \Orbit{x^-}\), by \cref{hip:elevators}, for every \(y \in (0,1)\) there exists \(n_0\) such that \(n \geq n_0\) implies \(f_{x_{-n}}^{-n}(y_0) > y\). In particular,
  \[y_{ - n} = f_{x_{-n}}^{-1} \circ \cdots \circ f_{x_{-1}}^{-1}(y_0)\]
  is such that \(y_{ - n} \to 1\). Therefore, \((x_{-n},y_{-n}) \to (x^-,1)\). Since \(\supp \beta_0\) is closed and \((x_{ - n},y_{ - n}) \in F^{-n}(\supp \beta_0)\), we conclude that \((x^-, 1) \in \supp \mu_0\). Now, by exactness, the set \(E = \bigcup_{n = 0}^{\infty}g ^{-1}(\{ x^- \})\) is dense in \(X\), and \(1\) is a fixed point of the \(f_x\), thus \(E \times \{ 1 \} \subseteq \supp \beta_0\). Taking the closure, we have \(\mathcal{A}_1 \subseteq \supp \beta_0\), and by \cref{lem:supp-segment}, it is the whole \(X \times I\).
\end{proof}

\begin{remark}
  With Pesin theory, it is possible to show that intermingled basins are present in the more general setting of \(g\) being a hyperbolic diffeomorphism. See the examples 3.2.1 and 3.2.2 of~\cite{uresNonrobustnessIntermingledBasins2018}.
\end{remark}

\begin{figure}[h]
  \centering
  \includegraphics[width=0.35\textwidth]{../figures/intermingled}
  \caption{Picture of the basins from \cref{ex:intermingled}.}
\end{figure}
...
The statement of the theorem below corresponds to~\cite[Theorem 3.2]{bonifantSchwarzianDerivativesCylinder2008}, but without the hypothesis that both basins have positive measure. The question of whether this hypothesis could be removed was left open in the article, and our proof below confirms that it is unnecessary.
...
% \subsection{Measures that project to the base}

% \begin{theorem}
%   Suppose that we are in the hypothesis of \cref{thm:neg-schwarz-sep-graph} and \(\nu\) is an invariant measure for \(F\) such that \((\pi_X)_*\nu = \mu\). Then, \(\nu \in \set{\mu_0,\mu_1,\mu_{\sigma}}\) where \(\mu_{\sigma} = (\graph \sigma)_*\mu\).
% \end{theorem}

% \subsection{Hausdorff dimension}

% An interesting question to consider is what is the Hausdorff dimension of the set \(\graph \xi\) in the situation of intermingled basins. Since this set is a graph, it is clearly at least \(1\) and at most \(2\).

% \todo{In this situation of intermingled basins, what is the Hausdorff dimension of the graph separating the basins, or of the basins themselves?}

% \todo{It should be strictly greater than 1: this is a corollary of regularity.}
...
\begin{proof}
  ...
  Let \((\eX, \eg, \emu)\) be the invertible extension of \((X, g, \mu)\), \(\pi \colon (\eX, \eg, \emu) \to (X, g, \mu)\) be the associated factor map, and \(\eF \colon \eX \times I \to \eX \times I\) be given by
  \begin{equation*}
    \eF(\ex, y) = (\eg(\ex), f_{\pi(\ex)}(y)).
  \end{equation*}
  Notice that \(\eF^{-1}(x, y) = (\eg^{-1}(x), f_{\pi(\eg^{-1}(x))}^{-1}(y))\). Since \(\pi_* \emu = \mu\) and \(\eg_* \emu = \emu\), by \cref{prop:schwarzian} we have \(f_{\pi(\eg^{-1}(\ex))}^{-1} \prec 0\) for almost every \(\ex \in \eX\). Also, by \cref{lem:inv-ext-ergodic}, \(\tilde{\mu}\) is ergodic for \(F\) (and therefore, also for \(F ^{-1}\)). Thus, we can apply \cref{thm:neg-schwarz-sep-graph} to \(F^{-1}\) and obtain a function \(\sigma \colon \eX \to I\) that separates the basins \(\eBu\) and \(\eBl\).

  The hyperbolic metric (which coincides with the Hilbert metric) on \((0, 1)\) is given by
  \[d_H(r, s) = \log \lvert \rho(0, r, s, 1) \rvert.\]
...
\begin{example}
  Let \(X = S^1\), \(g(x) = kx \pmod{1}\) and \(f_x(y) = q_{p(x)}^{-1}(y)\), where \(q_a(y) = y + ay(1 - y)\) and \(p(x) = \varepsilon \cos(2 \pi x)\) are defined as in \cref{ex:intermingled}. We claim that the Lebesgue measure \(\lambda\) on \(S^1\times I\) is the asymptotic measure for \(F\). To see this, notice that for every \(x \in X\), there is a small neighborhood \(U\) of \(x\) in which \(g\) has \(k\) inverse branches, which we label \(g_i^{-1} \colon U \to X\) for \(1 \le i \le k\). Therefore, for each branch, we have a local inverse
  \begin{equation*}
    F^{-1}_i(x,y) = (g_i^{-1}(x), y + \varepsilon \cos(2 \pi g^{-1}_i(x))y(1 - y)).
  \end{equation*}
  Since \((g_i^{-1})'(x) = k^{-1}\), the determinant of the Jacobian is given by
  \begin{equation*}
    \abs{d(F^{-1}_i)_{(x,y)}} =
      k^{-1} \cdot (1 + \varepsilon \cos (2\pi g_i^{-1}(x))(1 - 2y))
  \end{equation*}
  which across all branches, sums to
  \begin{equation*}
    \sum_{i = 1}^k
    \abs{d(F^{-1}_i)_{(x,y)}} =
    \frac{1}{k}\sum_{i = 1}^k
    (1 + \varepsilon \cos (2\pi g_i^{-1}(x))(1 - 2y)) = 1.
  \end{equation*}
  By the change of variables formula, this means that \(\lambda\) is preserved by \(F\).

  In \cref{ex:intermingled}, we calculated \(\SchD q_a \prec 0\) to be negative whenever \(a \neq 0\); by \cref{prop:schwarzian}, \(\SchD q_a^{-1} \succ 0\), so \(\SchD f_x \succ 0\) whenever \(\cos (2\pi x) \neq 0\). This means we can apply \cref{thm:pos-schwarz-asymptotic} to get an asymptotic measure \(\nu\) whose basin has full measure for \(\lambda\). But since \(\lambda\) is itself invariant, for all continuous \(\phi \in \Cs{S^1 \times I}\),
  \begin{equation*}
    \nu(\phi) = \lim_{n \to \infty} \int \Emp[n]{x}{\phi} \;d\lambda(x) = \lim_{n \to \infty} \frac{1}{n}\sum_{i = 0}^{n - 1} \int \phi \circ F^i \;d\lambda = \lambda(\phi).
  \end{equation*}
  This shows \(\nu = \lambda\).
\end{example}
