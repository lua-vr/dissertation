...
In this section we shall describe the construction of that flavor of example in a more general setting of skew products that also encompasses the original example. As before, let \(X\) be a compact metric space, \(g \colon X \to X\) be ergodic for \(\mu \in \Meas[1]{X}\) and \(F \colon X \times I \to X \times I\) a skew-product endomorphism of the form \(F(x,y) = (g(x), f_x(y))\). We shall understand by  ``volume'' the reference measure \(\mu \otimes \Leb\). As mentioned in \cref{lem:lyap-basin-posmeas}, under some mild hypothesis for \(F\), if the transverse Lyapunov exponents at the invariant boundaries \(\mathcal{A}_i = X \times \set{i}\), given by
\begin{equation*}
  \Lyap(\mathcal{A}_i) = \int \log f_{x}'(i) \;d \mu(x)
\end{equation*}
for \(i = 0,1\) are both negative, then both basins have positive volume. Assuming this, the main property that produces the intermingled phenomenon is the following hypothesis.
\begin{hypothesis}[Elevators]\label{hip:elevators}
  There exist two periodic points \(x^-\) and \(x^+\) in \(X\) whose unstable manifolds are dense, and such that for every  \(y \in (0,1)\), \(f_{x^-}(y) <  y\) and \(f_{x^+}(y) > y\).
\end{hypothesis}
...
In this section only, we fix \(X = \circl^1\), \(\mu = \Leb\), and \(g \colon \circl^1 \to \circl^1\) the map \(g(x) = kx\). Furthermore, we require the following hypothesis to hold.
...
\begin{theorem}\label{thm:inv-closed-contains-boundary}
  Suppose \cref{hip:elevators} holds. Then, any closed backwards-invariant set  \(K \subseteq X \times I\) with at least one point \((x,y) \in K\) such that \(y \in (0,1)\) contains both boundaries \(X \times \set{0,1}\).
\end{theorem}
...
\begin{figure}[h]
  ...
  \caption{Simulated picture of the basins of \cref{ex:intermingled}. It should not be taken too seriously, as it is evidently not possible to draw intermingled sets.}
\end{figure}
...
\subsection{Application: two random maps}
In~\cite{gharaeiRandomIntervalDiffeomorphisms2016}, the authors study some dynamical properties that are very similar to the ones considered in~\cite{bonifantSchwarzianDerivativesCylinder2008}, but for two random diffeomorphisms of the interval which are chosen \iid at random. Here we show that their proof of the existence of an invariant graph \parencite[Theorem 3.1]{gharaeiRandomIntervalDiffeomorphisms2016}, which spans several pages and relies on advanced results, can be considerably simplified using the ideas above.

Consider \(X = \set{+,-}[upper = \NN]\) and \(f_+, f_- \in \Diff[C^2]{I}\) diffeomorphisms preserving orientation such that \(f_+(y) > y\) and \(f_-(y) < y\) for all \(y \in (0,1)\).
