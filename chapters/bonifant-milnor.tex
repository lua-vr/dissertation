...
\begin{proof}
  ...
  \end{align*}
  This means that \((f^n_x)'(z) \to \infty\) uniformly on \(z \le y\) (because neither of \(N_2\) and \(\int a \;d\mu\) depend on \(0\le z \le y\)). This is impossible, because by the mean value theorem, \(f^n_x(y) = (f^n_x)'(z) \cdot y\) for some \(0\le z \le y\), while \(f^n_x(y) \le 1\) is bounded.
\end{proof}
...
\begin{definition}
  Let \(X\) be a compact metric space, \(f \colon X \to X\) and \(\mu\) be a reference Borel measure. We say \((X,f,\mu)\) displays intermingled basins if there exists two distinct invariant measures \(\mu_0 \neq \mu_1\) such that for each open set \(U \subseteq X\), we have \(\mu(\Basin{\mu_0} \cap U), \mu(\Basin{\mu_1} \cap U) > 0\).
\end{definition}
