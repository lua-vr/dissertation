...
\begin{hypothesis}[Elevators]\label{hip:elevators}
  There exists two fixed points \(x^-\) and \(x^+\) in \(X\) such that for every  \(y \in (0,1)\), \(f_{x^-}(y) <  y\) and \(f_{x^+}(y) > y\), We also assume that either their stable sets are dense, or alternatively, in order to also deal with the case of an expanding endomorphism (where the stable set is a point), we require that \(\bigcup_{i = 1}^{\infty} f^{-i}(\set{x^-})\), \(\bigcup_{i = 1}^{\infty} f^{-i}(\set{x^+})\) are both dense in \(X\).
\end{hypothesis}

Note that if we define the measures \(\mu_i(A) = \Leb(A \cap \mathcal{B}_i)\), then the basins are intermingled if, and only if, \(\supp \mu_i = X \times I\) for both \(i = 0,1\). We will show this is the case for \(\supp \mu_0\), as the proof for \(i = 1\) follows by symmetry. The result will follow from the following series of lemmas. 

\begin{lemma}
  We have \(\set{x^-} \times I \subseteq \supp \mu_0\).
\end{lemma}

\begin{proof}
  Let \(y \in (0,1)\) and \(U\) be an open set around \((x^-,y)\).

  
  If we define \(h(x) = \sup \set{y \in I \where (x,y) \in \mathcal{B}_0}\), by \cref{lem:basin-segment} we know \[\set{x} \times \interval[open right]{0}{h(x)} \subseteq \mathcal{B}_0 \cap (\set{x} \times I).\]
  Also note that \(\mathcal{B}_0\) is \(F\)-invariant and \(f_x(0) = 0\) for all \(x\), so \(\set{x \in X \where h(x) = 0}\) is \(g\)-invariant. Since by assumption \(\mathcal{B}_0\) has positive measure, \(h\) must be positive \(\mu\)-almost everywhere, so for any open set \(U \times \interval[open right]{0}{b}\),
  \[\Leb{U \times \interval[open right]{0}{b} \cap \mathcal{B}_0} = \int_U \min \set{h(x), b}\;d\mu(x) > 0.\]
\end{proof}
...
\begin{lemma}
  We have \(X \times \set{0} \subseteq \supp \mu_0\).
\end{lemma}
...
\subsection{Application: expanding maps on the real line}
