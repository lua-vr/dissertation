...
\chapter{Partially hyperbolic dynamics}\label{cha:part-hyperb-dynam}
...
Applying \cref{prop:jensen} to \(\Prob = \mu_x^{cu}\), \(\varphi(x) = \phi(x) =  - x\log x\) (which is strictly concave) and \(\psi_P(y) = \mu^u_p(P)\), for each \(P \in \mathcal{P}\) we have
\begin{equation*}
...
\begin{proof}
  ...
  \end{align*}
  Now, let \(n \to \infty\). Since \(\xi_2 \prec \bigvee_n f^{-n + 1}(\xi_1)\), the second term goes to zero. For the first term, define the set \(D_n = \set{x \in X \where f(\xi_1)(x) \subseteq f^{n + 1}(\xi_2)(x)}\).
...
Now, since \((f^n_c, \nu)\) is a factor of \((f^n,\mu)\), we have \(h_{\nu}(f_c^n) \le h_{\mu}(f^n)\). If \(\lambda^c(\mu) \le 0\), then the Ledrappier-Young formula of \cref{thm:leddrappier-young} tells us that \(h_{\mu}^u(f^n) = n h_{\mu}^u(f) = n h_{\mu}(f) = h_{\mu}(f^n)\), so we have equality in the equation above. By \cref{cor:cond-invprin}, the proof of \cref{thm:invariance-principle} is complete.
\end{proof}

\section{Applications to Bonifant-Milnor}

Consider the skew-product map \(F(x,y) = (g(x),f_x(y))\) on \(X \times I\) of \cref{cha:bonifant-milnor} again. If \(g \colon X \to X\) is a hyperbolic diffeomorphism and \(F\) is a diffeomorphism, then \(F\) will be partially hyperbolic~\cite{crovisierIntroductionPartiallyHyperbolic} and dynamically coherent, with center foliation given simply by \(\mathcal{F}^c(x,y) = \set{x} \times I\). This means we can apply the invariance principle to it.

\subsection{Center exponent of intermingled basins}

In \cref{sec:intermingled-basins}, we saw conditions for the basins of the two measures \(\mu_0,\mu_1\), supported at the boundaries \(X \times \set{0}\) and \(X \times \set{1}\), to be intermingled. Combined with \cref{thm:neg-schwarz-sep-graph}, we have the existence of a measurable function \(\sigma \colon X \to I\) whose graph separates the intermingled basins. Since the basins are intermingled, \(\sigma\) cannot be continuous; however, the \(u\)-holonomy maps \emph{are} continuous, so the ergodic measure \(\nu = (\graph \sigma)_*\mu\) supported at the graph cannot be invariant by unstable holonomies. The contrapositive of \cref{thm:invariance-principle} then implies its center Lyapunov exponent must be positive. This observation is sourced from~\cite[Section 4.2]{duarteMeasuresMaximalEntropy2022}.

% \begin{corollary}
%   Suppose \(g \colon X \to X\) is hyperbolic, \(F\) is a diffeomorphism and basins of \(\mu_0\) and \(\mu_1\) are intermingled for \(F\), and 
% \end{corollary}

\subsection{Absolute continuity of asymptotic measure}

\begin{question}
  We know the asymptotic measure \(\nu\) of \cref{thm:pos-schwarz-asymptotic} satisfies \((\pi_X)_*\nu = \mu\), and we can disintegrate \(\nu = \int \nu^c_x \;d\mu(x)\) for the partition \(\set{x}\times I\). What are general conditions for \(\nu^c_x \ll \Leb\) (that is, when is \(\nu\) absolutely continuous on the fibers)?
\end{question}

The proposition below, sourced from~\cite[Proposition 6.7]{crovisierInvariancePrincipleNoncompact2023}, gives a suggestive answer. Its proof uses the accessibility hypothesis and the uniformly Lipschitz property of the stable and unstable holonomy maps, together with the invariance of the center measures, to compare the Lebesgue and \(\nu^c_x\)-volume of balls on different points.

\begin{definition}
  We say a partially hyperbolic diffeomorphism \(f \colon M \to M\) is \emph{accessible} if every \(x, y \in M\) can be connected by a path which is the union of finitely many \(C^1\) paths tangent to \(\mathcal{W}^{s,u}\) leaves (the strong-stable and strong-unstable leaves, see~\cite{crovisierIntroductionPartiallyHyperbolic} for a definition).
\end{definition}
