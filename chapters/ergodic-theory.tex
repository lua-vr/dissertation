...
\end{equation*}
and it is this quality of being defined by the values observed on a finite number of steps along an orbit that should justify the nomenclature \textit{empirical}.

The direct relation to Birkhoff averages also highlights these objects as having a central role in ergodic theory. One of the motivating problems of the field is the \textit{Ergodic Hypothesis}, formulated by the Austrian physicist Ludwig Boltzmann to give mathematical justification to the kinetic theory of gases \cite{vianaFoundationsErgodicTheory2016}.
...
For each invariant measure \(\mu \in \Meas[f]{X}\), we can ask whether the empirical measures of a point will converge weakly to it. The set \(\Basin{\mu} \subseteq X\) of points with this property is called the \textbf{basin} of the measure \(\mu\).

It may happen that the set of points whose empirical measures do not converge is large, in the sense that it has positive volume. Systems with this property are said to exhibit \textbf{historical behavior}, and the existence of diffeomorphisms exhibiting historical behavior in a robust way is an important problem in dynamics.
...
\begin{lemma}
  The partition \(\bot\) of all points of a compact metric space \(\cms\) is measurable.
\end{lemma}
\begin{proof}
  Let \(\set{U_i}_{i \in \NN}\)
  foo
\end{proof}
...
\end{corollary}
\begin{proof}
  Under the hypothesis, we can define a metric on \(\pP[eqc]\) by letting \(d(P_1,P_2)\) be the distance between the compact sets \(P_1, P_2 \in \pP[eqc]\). With this metric,  \(\pP \colon \cms \to \pP[eqc]\) is a contraction, so \(\pP[eqc]\) is a compact metric space as well. Consequently, the partition \(\bot\) of points of \(\pP[eqc]\) is measurable, thus so is its preimage \(\pP\).
\end{proof}
...
\section{Pesin entropy formula (tying things together)}
