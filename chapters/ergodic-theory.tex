...
We can relate different dynamical systems by the concept of \emph{factors}, or \emph{extensions}, expressing a sense in which one system is a simplification of another. In the categorical language we have been developing, an extension is a natural transformation \(F \Rightarrow G\) between two dynamical systems \(F\) and \(G\). In this situation, we then say that \(G\) is a \emph{factor} of \(F\).

\begin{definition} Let $(X, \mathcal{B}_X, \mu, T)$ and $(Y,\mathcal{B}_Y, \nu, S)$ be measure-preserving transformations. 
  $(Y,\mathcal{B}_Y, \nu, S)$ is a \textbf{factor} of $(X, \mathcal{B}_X, \mu, T)$ if there exists a measure-preserving map $\phi: X \to Y$ such that 
  \[
    \begin{tikzcd}
      X \arrow[r, "T"] \arrow[d, "\phi"] & X \arrow[d, "\phi"] \\
      Y \arrow[r, "S"] & Y
    \end{tikzcd}
  \]
  commutes, that is,
  $$ \phi \circ T(x) = S \circ \phi (x),\,\,\, \forall x \in X.$$    
\end{definition}
  

\begin{example}
  Every system admits a trivial extension:
  \[
    \begin{tikzcd}
...
``A factor of a measure-preserving system determines uniquely, and is uniquely determined by, an invariant sub-$\sigma$-algebra''

\begin{itemize}
\item $\mathcal{A} \subset \mathcal{B}_X$, with $T^{-1}(\mathcal{A}) = \mathcal{A}$(mod $\mu$)
\end{itemize}

In the previous example, the  sub-\(\sigma\)-algebra with respect to the extension is \(\{ \varnothing, X \}\).

\begin{lemma}
  Let $(X, \mathcal{B}_X, \mu, T)$ and $(Y,\mathcal{B}_Y, \nu, S)$ be invertible systems, and let $\phi:X \to Y$ be a factor map. 
  Then $\mathcal{A} = \phi^{-1}(\mathcal{B}_Y) \subset \mathcal{B}_X$ is an invariant sub-$\sigma$-algebra.
\end{lemma}

\begin{theorem}
    Let $(X, \mathcal{B}_X, \mu, T)$ be a measure-preserving Borel probability system. 
    Suppose there is $\mathcal{A}$ a sub-$\sigma$-algebra $T-$invariant. 
    Then there is a measure-preserving system $(Y,\mathcal{B}_Y, \nu, S)$ on a Borel probability space and a factor map $\phi:X \to Y$ with $\mathcal{A} = \phi^{-1}(\mathcal{B}_Y)$ (mod $\mu$). 
\end{theorem}
...
Consider the system $\mathcal{X} = (X, \mathcal{B}_X, \mu, T)$ such that
\begin{itemize}
\item $X = \mathbb T^2$;
\item $\mathcal{B}_X = \mathcal{B} \times \mathcal{B}$, where $\mathcal{B}$ is the Borel $\sigma$-algebra of $\mathbb T$;
\item $T: \mathbb T^2 \to \mathbb T^2$ defined by $T(x,y) = (x + \alpha, y+x)$, with $\alpha$ irrational;
\item $m_X = m \times m$, where $m$ is the Lebesgue measure in $\mathbb T$.
\end{itemize}

\begin{frame}{Example of extension}
  The system $\mathcal{Y} = (Y,\mathcal{B}_Y, \nu, S)$ defined by
  \begin{itemize}
  \item $Y = \mathbb T$;
  \item $\mathcal{B}_Y = \mathcal{B}$;
  \item $S: \mathbb T \to \mathbb T$ defined by $S(x) = (x + \alpha)$;
  \item $m_Y = m$.
  \end{itemize}
  is a factor of $\mathcal{X}$.

  In fact, consider the projection $\pi:X \to Y$ such that $\pi(x,y) = x$, so 
  % \[
  %   \begin{tikzcd}
  %     \mathbb T^2 \arrow[r, "T"] \arrow[d, "\pi"] & \mathbb T^2 \arrow[d, "\pi"] \\
  %     \mathbb T \arrow[r, "S"] & \mathbb T
  %   \end{tikzcd}
  % \]

  $\pi_* m_x = m = m_y$ and $\pi^{-1}(A) = A \times \mathbb T \in \mathcal{B_X}$ for all $A \in \mathcal{B_Y}$  
\end{frame}


\begin{lemma}
  Suppose \(p \colon (X, f) \to (Y, g)\) is an extension. Then, if \(\mu \in \Meas[f]{X}\) is ergodic for \(f\) then \(p_*\mu \in \Meas[g]{Y}\) is ergodic for \(g\).
\end{lemma}
...
\begin{theorem}
  Let \((X,\mathcal{B},\mu,T)\) be a measure-preserving system on a Borel space.
\end{theorem}
...
\subsection{Constructing the invertible extension}
...
\todo{Improve this, this is the main concept of the dissertation, so it deserves more love.}
...
\begin{proof}
  ...
  \end{equation*}
  which is a metric inducing the weak* topology on the set of probability measures (see~\ref{prop:weak-star-metrizable}). Since it is a metric, there are two open sets \(U_1 \supseteq K_1, U_2 \supseteq K_2\) with positive distance \(s > 0\) from each other. We claim \(\lim_{n \to \infty} d(\Emp[n]{x}, \Emp[n + 1]{x}) = 0\), which would contradict \(s > 0\) because the definition of \(\Meas{x}\) implies \(\Emp[n]{x} \in U_1\) and \(\Emp[n + 1]{x} \in U_2\) for infinitely many \(n\).
  Indeed,
  \begin{align*}
    d(\Emp[n](x), \Emp[n + 1](x))
