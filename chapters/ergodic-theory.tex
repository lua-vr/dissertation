...
Reciprocally, for each invariant measure \(\mu \in \Meas[f]{X}\), we can ask which points in \(X\) have empirical measures converging to \(\mu\). The set \(\mathcal{B}(\mu) \subseteq X\) of points with this property is called the \emph{basin} of the measure \(\mu\).
...
\begin{proof}
  ...
  Therefore, \(p_*\mu(A) = \mu(p^{-1}(A)) \in \set{0,1}\).
\end{proof}
...
\begin{proof}
  Suppose \(\mu\) is ergodic but \(\tilde{\mu}\) is not; then, by the characterization of ergodic measures as extremal points~\cite[Proposition 4.3.2.]{vianaFoundationsErgodicTheory2016}, we can write \(\tilde{\mu} = \lambda\mu_0 + (1 - \lambda)\mu_1\) for two different invariant measures \(\mu_0 \neq \mu_1\). Let \(\nu_0 = \pi_*\mu_0\) and \(\nu_1 = \pi_*\mu_1\), so that \(\mu = \lambda\nu_0 + (1 - \lambda)\nu_1\) by linearity. By ergodicity, \(\nu_0 = \nu_1 = \mu\), contradicting the uniqueness of the measure \(\tilde{\mu}\) such that \(\pi_*\tilde{\mu} = \mu\).
\end{proof}
...
\begin{proposition}
  If \((X,\mathcal{B},\mu,f)\) is a measure-preserving system and \(\xi\) is a partition of \(X\) with finite entropy, the entropy of \(f\) with respect to \(\xi\) is equivalently equal to
  \begin{equation*}
    h_{\mu}(f,\xi) =
...
The numbers \(\delta_i\) are called the \emph{dimension of \(\mu\) on \(W^i\)-manifolds}.
