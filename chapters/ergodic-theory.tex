...
\todo{The fundamental tools in ergodic theory.}
...
\begin{proof}
  ...
  Let \(\widetilde{X} \subseteq X^{\ZZ}\) be the subset defined by
  \begin{equation*}
    \widetilde{X} = \bigcap_{i \in \ZZ} \set{ x \in X^{\ZZ} \where x_{i + 1} = f(x_{i})},
  \end{equation*}
  which is closed in the product topology by definition.

  , with \(\pi \colon \widetilde{X} \to X\) being the projection on the zeroth coordinate, that is, \(\pi(\dots,x_{-1},x_0,x_1,\dots) = x_0\).

  First, define \(\tilde{f}\) to be
\end{proof}

\begin{lemma}
  Let \(\pi \colon (\widetilde{X}, \tilde{f}, \tilde{\mu}) \to (X, f, \mu)\) be a natural invertible extension. Then, we have a converse for \cref{lem:factor-ergodic}: if \(\mu\) is ergodic for \(f\), then \(\tilde{\mu}\) is ergodic for \(\tilde{f}\).
\end{lemma}
...
It may happen that the set of points whose empirical measures do not converge is large, in the sense that it has positive volume. Systems with this property are said to exhibit \emph{historical behavior}, and the existence of diffeomorphisms exhibiting historical behavior in a robust way is an important open problem in dynamics.
...
\begin{lemma}\label{lem:erg-iff-basin-full-meas}
  If \(\mu\) is invariant for \(f\), then \(\mu(\Basin{\mu}) \in \set{0,1}\). Moreover, \(\mu(\Basin{\mu}) = 1\) if, and only if, \(\mu\) is ergodic for \(f\).
\end{lemma}
